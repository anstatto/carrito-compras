...
export async function GET() {
  try {
    ...
    // Verificar si el caché existe y devolverlo si es válido
    if (cache.usersCache) {
      return NextResponse.json(cache.usersCache);
    }
    ...
    // Actualizar caché
    cache.usersCache = users.map((user) => ({
      id: user.id,
      nombre: user.nombre,
...
export async function POST(request: Request) {
  try {
    ...
    // Opcional: Actualizar el caché después de crear un nuevo usuario
    if (cache.usersCache) {
      cache.usersCache.push({
        id: user.id,
        nombre: user.nombre,
        apellido: user.apellido,
        email: user.email,
